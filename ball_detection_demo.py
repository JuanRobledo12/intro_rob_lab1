...
while True:
    ...
    blurFrame = cv.GaussianBlur(grayFrame, (7,7), 1.5)
    ...
    circles = cv.HoughCircles(blurFrame, cv.HOUGH_GRADIENT, 1.5, 100, param1=200,
    ...
    if circles is not None:
        ...
        for i in circles[0,:]:
            ...
            txt_mark = "x: " + str(chosen[0]) + " y: " + str(chosen[1]) 
            cv.putText(frame, txt_mark, org=(chosen[0], chosen[1]),fontFace=cv.FONT_HERSHEY_SIMPLEX, fontScale=1 ,color=(102, 255, 51), thickness=2, lineType=cv.LINE_AA)
            prevCircle = chosen
